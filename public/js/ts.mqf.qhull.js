      ...
      function traverse(node, depth) {
        if (node.childNode && node.childNode.length) {
          for (var i = 0; i < node.childNode.length; i++) {
            if (depth == 2) {
              // 차시이면
              if (node.childNode[i].Content)
                node.childNode[i].Content.idLesson = node.Id;
            }
    ...
    function(ev) {
      ...
      if (ev.data && ev.data.pando && ev.data.pando.event == "sensor") {
        if (_this._cfg.listener && _this._cfg.listener.sensor) {
          ...
          var msg =
            '<div style="color:yellow;">[Sensor] ' + sensor.Sensor + "</div>";

          msg +=
            '<div style="padding-left:1em;"><span style="color:yellow;">[Data]</span><br/>' +
            escapeHTML(JSON.stringify(sensor.Data)) +
            "</div>";
          if (sensor.where)
            msg +=
              '<div style="padding-left:1em;"><span style="color:yellow;">[Where]<br/></span>' +
              escapeHTML(JSON.stringify(sensor.where)) +
              "</div>";
            ...
            for (var j = 0; j < sensor.Data[i].Q.length; j++) {
              if ((meta = sensor.Data[i].Q[j].meta) && meta.sentence) {
                for (var k = 0; k < meta.sentence.length; k++) {
                  ...
                  sentenceMap[sensor.Data[i].Q[j].IdQ] =
                    sentenceMap[sensor.Data[i].Q[j].IdQ] || {};
          ...
          if (sentenceArray.length && _this._cfg.storage.sentence) {
            _this._cfg.storage.sentence.call(_this, sentenceArray, function(
              data
            ) {
              var msg = "";
              ...
              for (var i in sentenceMap) {
                msg +=
                  '<div style="padding-left:1em;"><span style="color:yellow;">[Sentence] ' +
                  i +
                  "</span><br/>";
                for (var j in sentenceMap[i]) {
                  msg +=
                    (data[j] && data[j].Text
                      ? escapeHTML(data[j].Text)
                      : "UNKNOWN!") + "<br/>";
                }
                msg += "</div>";
              }
            ...
            for (var j = 0; j < sensor.Data[i].Q.length; j++) {
              if ((meta = sensor.Data[i].Q[j].meta) && meta.subject) {
                for (var k = 0; k < meta.subject.length; k++) {
                  ...
                  subjectMap[sensor.Data[i].Q[j].IdQ] =
                    subjectMap[sensor.Data[i].Q[j].IdQ] || {};
          ...
          if (subjectArray.length && _this._cfg.storage.subject) {
            _this._cfg.storage.subject.call(_this, subjectArray, function(
              data
            ) {
              var msg = "";
              ...
              for (var i in subjectMap) {
                msg +=
                  '<div style="padding-left:1em;"><span style="color:yellow;">[Subject] ' +
                  i +
                  "</span><br/>";
                for (var j in subjectMap[i]) {
                  msg +=
                    (data[j] ? escapeHTML(data[j].title) : "UNKNOWN!") + " ";
                }
                msg += "</div>";
              }
